...
export class GroupChart {
  ...
  update(source) {
    ...
    node1Enter
      .append('rect')
      .attr('width', d => (d.depth === 0 ? (d.data.name.length + 2) * 16 : this.config.rectWidth))
      .attr('height', d => (d.depth === 0 ? 50 : this.config.rectHeight))
      .attr('x', d => {
        if (d.depth === 0) {
      ...
      .attr('stroke', d => (d.depth === 0 ? '#ff6c52' : '#6d8dfe'))
      .attr('fill', d => (d.depth === 0 ? '#ff6c52' : '#FFF'))
    ...
    node1Enter
      ...
      .attr('height', 20)
      .attr('x', d =>
        d.depth === 0 ? (-(d.data.name.length + 2) * 16) / 2 : -this.config.rectWidth / 2
      )
      .attr('y', d => (d.depth === 0 ? 24 : 15))
      .attr('fill', d => {
        if (!d.data.type || d.depth === 0) return 'transparent'
        return '#6d8dfe'
      })
    ...
    node1Enter
      ...
      .attr('y', -50)
      .text(d => {
        if (d.depth !== 0) return d.data.percent
      })
      .attr('fill', '#128bed')
      .style('font-family', '黑体')
      .style('font-size', 14)
    ...
    node2Enter
      .append('rect')
      .attr('width', d => {
        if (d.depth === 0) return (d.data.name.length + 2) * 16
        return this.config.rectWidth
      })
      .attr('height', this.config.rectHeight)
      .attr('x', d => {
        if (d.depth === 0) return (-(d.data.name.length + 2) * 16) / 2
        return -this.config.rectWidth / 2
      })
      .attr('y', d => (d.depth === 0 ? -26 : -this.config.rectHeight / 2))
      .attr('rx', 1)
      .attr('stroke-width', 1.5)
      .attr('stroke', d => (d.depth === 0 ? '#ff6c52' : '#ff9b29'))
      .attr('fill', d => (d.depth === 0 ? '#ff6c52' : '#FFFFFF'))
      .on('click', (e, d) => this.nodeClickEvent(e, d))
    ...
    node2Enter
      ...
      .attr('y', d => (d.depth === 0 ? 24 : 15))
      .attr('fill', d => (d.depth === 0 ? '#fff' : '#ff9b29'))

    // 文本主标题
    node2Enter
      .append('text')
      .attr('x', 0)
      .attr('y', d => (d.depth === 0 ? 2 : -10))
      .attr('text-anchor', 'middle')
      .text(d => {
    ...
    node2Enter
      ...
      .text(d => {
        if (d.depth !== 0) {
          let subTitle = d.data.name.substring(11)
          return subTitle.length > 10 ? subTitle.substring(0, 10) + '...' : subTitle
        }
      })
